...
class GraphRenderer {
    constructor() {
        ...
        this.BASE_SCALE = 50; // базовый масштаб для R=1
        this.currentR = null;
    }
    ...
    getScale(r) {
        ...
        // Масштаб адаптируется так, чтобы график всегда занимал разумную часть области
        // Максимальный размер области = 250 пикселей (половина от 500)
        const maxSize = 250;
        const scale = maxSize / r;
        return Math.max(scale, 20); // Минимальный масштаб 20 пикселей на единицу
    }
    ...
    drawGrid(svg, scale = this.BASE_SCALE) {
        const gridStep = scale / 2; // Сетка каждые 0.5 единицы
    ...
    drawPoints(svg, r, resultsData) {
        ...
        const filteredResults = resultsData.filter(result => {
            ...
            // Используем приблизительное сравнение для дробных чисел
            return Math.abs(resultR - rNum) < 0.0001;
        });
